...
Name-US: Scale To Fit Comp
Version: 1.0.0
Description-US: Scales foreground image to fit background image's width and height.
...
def fit_comp_height(tool) -> None:
    ...
    size_x  = bg_width / fg_width
    size_y  = bg_height / fg_height

    flow = comp.CurrentFrame.FlowView
    flow.Select()
    x, y = flow.GetPosTable(tool).values()
    transform_node = comp.AddTool("Transform", x, y-1)
    transform_node.SetInput("UseSizeAndAspect", 0)
    transform_node.SetInput("XSize", size_x)
    transform_node.SetInput("YSize", size_y)
    transform_node.SetAttrs({"TOOLS_Name": "FitToComp"})
